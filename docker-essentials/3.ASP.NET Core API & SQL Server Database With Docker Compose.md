...
For some odd reason , I noticed , that the docker file we edited , changed some things back.. Stubborn .. so double check to make sure its hasnt changed then continue below. Also make sure to save changes
...
Sometimes the port shows , other times naah lol , not sure whats up with that , but usually I would stop the container , delete that , since we built it already , then hit the debug button in visual studio and run it through there to see the ports its being hosted on.

But there is a cheatcode.

`https://localhost:8081/` or `https://localhost:8080/`. 

Since we specified it and paste it in the browser, usually when you run this for the first time (Tested on my sisters laptop), you can actually get a port back within Docker Desktop.
